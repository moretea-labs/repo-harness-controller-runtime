...
Start with [Tutorial 2: Connect ChatGPT](docs/tutorials/02-connect-chatgpt.md). After connecting, call `rh_status`, then use `rh_context` for the selected repository. The default `advanced` toolset is a stable repair-capable schema. Prefer `rh_status`, `rh_access`, `rh_context`, `rh_work`, and `rh_inbox`; `rh_context` reports capability groups for repository core, Git, Issue/Task, Campaign, Browser, iOS, plugins, evidence, and runtime maintenance so ChatGPT can choose the existing typed atomic tool. The current MCP transport still uses a static schema and cannot load domain schemas during a conversation, so Direct Edit, command, Git, Agent, Campaign, iOS, plugin, artifact, and recovery tools remain available. `core` is a compatibility alias and `full` is exhaustive legacy exposure. Reconnect or rescan only when tool names or schemas actually change; capability metadata changes alone do not change `tools/list`.
