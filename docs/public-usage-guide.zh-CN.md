...
- `cloudflared`：使用内置 Cloudflare quick/named tunnel
- ngrok：使用外部临时或保留域名 HTTPS 隧道
...
ChatGPT 需要通过 HTTPS 访问 MCP。选择一种方案即可。
...
免费临时域名可能变化。长期使用请选择 ngrok reserved/static domain、Cloudflare named tunnel 或其他稳定 HTTPS 反向代理。
...
Controller tool surface 或工具描述升级后，应在 ChatGPT 中 Refresh Connector metadata。
