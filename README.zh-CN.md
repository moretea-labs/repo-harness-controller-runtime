...
面向 ChatGPT 主导的本地开发，repo-harness 现在提供持久化 Controller，而不是把整个需求一次性交给一个大 Goal：

- ChatGPT 可以读取普通项目源码、维护依赖明确的 Issue/Task、启动受限的 Codex/Claude Run、检查 diff，并记录 Verification Gate 证据。
- `http://127.0.0.1:8766/` 的本地可视化控制台展示任务看板、审批、实时日志、Run 事件、已用/剩余时间、终止和重试。
- 本地 Agent 默认时限为 60 分钟，可明确设置至最长 12 小时；参数会原样落盘，非法值直接报错，不再静默回退到 120 秒。
- MCP 健康指纹 `controller-local-execution-v2` 可以直接识别 ChatGPT 是否仍缓存旧 Planner 工具面，或 8765 端口是否运行了错误版本。

详见 [ChatGPT MCP 接入](docs/repo-harness-chatgpt-mcp-setup.md) 和 [Local Execution Bridge](docs/repo-harness-local-execution-bridge.md)。
...
Controller 不暴露任意 shell 参数，也不会自动 merge 或 push。本地 Agent Run 会持久化并默认在独立 Git worktree 中执行；GitHub 模式可以把 Issue/Task 子 Issue 发布到 Project，并启动可在 Agents 页面和草稿 PR 中观察的 Copilot 云端 Session。Run 成功只会把 Task 推进到 review，ChatGPT 仍需检查本地 diff 或 GitHub PR，记录检查与验收证据并通过 Verification Gate，之后才能明确接受。
...
`controller` profile 启动 keepalive 时还会默认启动仅监听本机的可视化控制台：`http://127.0.0.1:8766/`。你可以在页面中直接启动 ready Task、创建小型 Codex/Claude 会话、批准本地 Job、查看实时 Run 日志和运行命名检查，不需要反复手动执行脚本。使用 `--open-local-ui` 可在启动时自动打开页面，使用 `--no-local-ui` 可关闭该能力。
