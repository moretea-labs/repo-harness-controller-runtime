...
export function stageSupervisorRelease(input: { controllerHome: string; repoRoot: string; sourceRoot?: string }): SupervisorStagedRelease {
  ...
  writeFileSync(join(releasePath, 'manifest.json'), `${JSON.stringify({ schemaVersion: 1, releaseRevision: revision, sourceRoot, builtAt: new Date().toISOString(), entrypoint: 'supervisor.js', runtimeEntrypoint: 'repo-harness.js', daemonEntrypoint: 'daemon.js', workerEntrypoint: 'worker.js', processRunnerEntrypoint: 'process-runner.js', browserHandoffHostEntrypoint: 'browser-handoff-host.js', capabilities: ['staged_rollout_release', 'browser_handoff_host', 'independent_process_runner'] }, null, 2)}\n`, { encoding: 'utf8', mode: 0o600 });
  for (const executable of ['supervisor.js', 'repo-harness.js', 'daemon.js', 'worker.js', 'process-runner.js', 'browser-handoff-host.js']) {
