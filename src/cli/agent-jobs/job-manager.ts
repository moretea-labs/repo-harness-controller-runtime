...
import { closeSync, existsSync, mkdirSync, openSync, readFileSync, readdirSync, writeFileSync, appendFileSync } from 'fs';
import { dirname, join, relative } from 'path';
import { spawn } from 'child_process';
import { fileURLToPath } from 'url';
import { runProcess } from '../../effects/process-runner';
import { getGitHubAgentSession, getGitHubAgentSessionLog, startGitHubAgentSession } from '../github/session';
...
import type { AgentJobEvent, AgentJobMeta, AgentJobStatus, AgentJobWorkerConfig } from './types';
...
export function appendAgentJobEvent(repoRoot: string, runId: string, event: Omit<AgentJobEvent, 'at'> & { at?: string }): void {
  const path = eventPath(repoRoot, runId);
  mkdirSync(dirname(path), { recursive: true });
  appendFileSync(path, `${JSON.stringify({ ...event, at: event.at ?? new Date().toISOString() })}\n`, 'utf-8');
}

function createWorktree(repoRoot: string, issueId: string, task: ControllerTask, runId: string): { path: string; branch: string | null; baseRevision: string | null } {
  const gitCheck = runProcess('git', ['rev-parse', '--is-inside-work-tree'], { cwd: repoRoot, timeoutMs: 10_000, maxOutputBytes: 8 * 1024 });
  if (!gitCheck.ok || gitCheck.stdout.trim() !== 'true') return { path: repoRoot, branch: null, baseRevision: null };
  const revision = runProcess('git', ['rev-parse', 'HEAD'], { cwd: repoRoot, timeoutMs: 10_000, maxOutputBytes: 8 * 1024 });
  if (!revision.ok) throw new Error(`failed to resolve task base revision: ${revision.error || revision.stderr}`);
  ...
  const result = runProcess('git', ['worktree', 'add', '-b', branch, absoluteWorktree, 'HEAD'], {
    cwd: repoRoot,
    timeoutMs: 60_000,
    maxOutputBytes: 64 * 1024,
  });
  if (!result.ok) throw new Error(`failed to create task worktree: ${result.error || result.stderr}`);
...
export function taskPrompt(
  ...
  const executionRules = provider === 'github'
    ? [
  ...
  return [
    ...
    '',
    `- Repository: ${repoRoot}`,
    provider === 'local' ? `- Worktree: ${worktree}` : '- Execution: GitHub Copilot cloud agent session',
    ...
    '## Acceptance criteria',
    '',
    ...(task.acceptanceCriteria.length ? task.acceptanceCriteria.map((item) => `- ${item}`) : ['- Complete the stated objective without unrelated changes.']),
    '',
    '## Required checks',
    '',
    ...(task.checks.length ? task.checks.map((item) => `- ${item}`) : ['- Run the smallest relevant verification available in the repository.']),
    '',
    '## Execution rules',
    '',
...
function baseMeta(opts: StartTaskJobOptions, task: ControllerTask, runId: string, paths: {
  promptPath: string;
  stdoutPath: string;
  stderrPath: string;
  resultPath: string;
  eventsPath: string;
}, isolation: { path: string; branch: string | null; baseRevision: string | null }, provider: 'local' | 'github'): AgentJobMeta {
  return {
    schemaVersion: 2,
    runId,
...
export function startTaskJob(opts: StartTaskJobOptions): AgentJobMeta {
  const timeoutMs = normalizeAgentTimeoutMs(opts.timeoutMs, { defaultMs: DEFAULT_AGENT_TIMEOUT_MS, maxMs: MAX_AGENT_TIMEOUT_MS });
  opts = { ...opts, timeoutMs };
  ...
  if (task.status !== 'ready' && task.status !== 'changes_requested') throw new Error(`task is not dispatchable from status ${task.status}`);
  ...
  const isolation = provider === 'local' && opts.isolate !== false
    ? createWorktree(opts.repoRoot, opts.issueId, task, runId)
    : { path: opts.repoRoot, branch: null, baseRevision: null };
  const paths = {
  ...
  const prompt = taskPrompt(issue.title, issue.summary, task, opts.repoRoot, isolation.path, provider);
  ...
  const meta = baseMeta(opts, task, runId, paths, isolation, provider);
  const absoluteMetaPath = metaPath(opts.repoRoot, runId);
  writeJson(absoluteMetaPath, meta);
  appendAgentJobEvent(opts.repoRoot, runId, { type: 'run_created', message: `${agent} Run created.` });

  if (provider === 'github') {
    try {
      const session = startGitHubAgentSession(opts.repoRoot, {
        prompt,
        repo: opts.githubRepo ?? (issue.github ? `${issue.github.owner}/${issue.github.repo}` : undefined),
      ...
      appendAgentJobEvent(opts.repoRoot, runId, {
        type: meta.status === 'succeeded' ? 'run_succeeded' : meta.status === 'failed' ? 'run_failed' : meta.status === 'waiting_for_user' ? 'run_waiting' : 'run_started',
      ...
      updateTask(opts.repoRoot, issue.id, task.id, {
        status: meta.status === 'succeeded' ? 'review' : meta.status === 'failed' ? 'blocked' : meta.status === 'waiting_for_user' ? 'blocked' : 'running',
        runId,
      ...
      writeJson(absoluteMetaPath, meta);
      appendAgentJobEvent(opts.repoRoot, runId, { type: 'run_failed', message: meta.error });
      updateTask(opts.repoRoot, issue.id, task.id, { status: 'blocked', runId, note: `${runId} failed to start: ${meta.error}` });
      return meta;
    }
  }
  ...
  const workerEntry = fileURLToPath(new URL('./job-worker.ts', import.meta.url));
  ...
  meta.status = 'running';
  meta.startedAt = new Date().toISOString();
  meta.deadlineAt = new Date(Date.parse(meta.startedAt) + timeoutMs).toISOString();
  meta.lastHeartbeatAt = meta.startedAt;
  writeJson(absoluteMetaPath, meta);
  appendAgentJobEvent(opts.repoRoot, runId, { type: 'run_started', message: `Local ${agent} worker started.` });
  updateTask(opts.repoRoot, issue.id, task.id, { status: 'running', runId, note: `Dispatched ${runId} to ${meta.agent}.` });
  return meta;
}
...
function syncTaskStatus(repoRoot: string, meta: AgentJobMeta): void {
  try {
    ...
    const targetStatus = meta.status === 'succeeded' ? 'review'
      : ['failed', 'unknown'].includes(meta.status) ? 'blocked'
        : meta.status === 'waiting_for_user' ? 'blocked'
          : meta.status === 'cancelled' ? 'cancelled'
            : undefined;
    if (task && targetStatus && task.status !== targetStatus) {
      updateTask(repoRoot, meta.issueId, meta.taskId, {
        status: targetStatus,
        note: targetStatus === 'review' ? `${meta.runId} finished and is ready for review.` : `${meta.runId} requires attention: ${meta.error ?? meta.status}`,
      });
    }
  } catch (_error) {
...
export function getAgentJob(repoRoot: string, runId: string): AgentJobMeta & { stdoutTail: string; stderrTail: string; timing: { elapsedMs: number; remainingMs: number | null; overdue: boolean } } {
  ...
  meta.eventsPath = meta.eventsPath ?? relative(repoRoot, eventPath(repoRoot, runId)).replace(/\\/g, '/');

  if (meta.provider === 'github' && meta.github && ['queued', 'running', 'waiting_for_user', 'unknown'].includes(meta.status)) {
    try {
      const session = getGitHubAgentSession(repoRoot, meta.github.owner, meta.github.repo, meta.github.taskId);
      ...
      if (meta.status === 'succeeded' || meta.status === 'failed' || meta.status === 'cancelled') meta.finishedAt = meta.finishedAt ?? new Date().toISOString();
      writeJson(path, meta);
      if (previous !== meta.status) {
        const eventType = meta.status === 'succeeded' ? 'run_succeeded' : meta.status === 'failed' ? 'run_failed' : meta.status === 'waiting_for_user' ? 'run_waiting' : meta.status === 'cancelled' ? 'run_cancelled' : 'log_updated';
        appendAgentJobEvent(repoRoot, runId, { type: eventType, message: `GitHub session state changed to ${session.state}.`, data: { url: meta.github.url, pullRequestUrl: meta.github.pullRequestUrl } });
      }
    } catch (error) {
      meta.error = error instanceof Error ? error.message : String(error);
      writeJson(path, meta);
    ...
    if (['queued', 'running'].includes(meta.status) && existsSync(resultAbsolute)) {
      const result = readJson<{ ok: boolean; exitCode: number | null; error?: string; finishedAt: string }>(resultAbsolute);
      const previous = meta.status;
      ...
      writeJson(path, meta);
      if (previous !== meta.status) appendAgentJobEvent(repoRoot, runId, { type: result.ok ? 'run_succeeded' : 'run_failed', message: result.ok ? 'Local worker finished.' : (result.error ?? `exit ${result.exitCode}`) });
    } else if (meta.status === 'running' && !isAlive(meta.workerPid)) {
      meta.status = 'unknown';
      meta.error = meta.error ?? 'worker process is no longer running and no result file was produced';
      writeJson(path, meta);
    }
  }
  ...
  const startMs = meta.startedAt ? Date.parse(meta.startedAt) : Date.parse(meta.createdAt);
  const endMs = meta.finishedAt ? Date.parse(meta.finishedAt) : Date.now();
  const deadlineMs = meta.deadlineAt ? Date.parse(meta.deadlineAt) : (meta.timeoutMs ? startMs + meta.timeoutMs : NaN);
  const remainingMs = Number.isFinite(deadlineMs) && !meta.finishedAt ? Math.max(0, deadlineMs - Date.now()) : null;
  return {
    ...meta,
    ...
    timing: {
      ...
      remainingMs,
      overdue: Number.isFinite(deadlineMs) && !meta.finishedAt && Date.now() > deadlineMs,
    },
  };
}
...
export function getAgentJobEvents(repoRoot: string, runId: string, limit = 200): AgentJobEvent[] {
  getAgentJob(repoRoot, runId);
  const path = eventPath(repoRoot, runId);
  if (!existsSync(path)) return [];
  return readFileSync(path, 'utf-8').split(/\r?\n/).filter(Boolean).map((line) => readJsonLine<AgentJobEvent>(line)).slice(-Math.min(Math.max(limit, 1), 1000));
}
...
export function getAgentJobLog(repoRoot: string, runId: string, follow = false): { runId: string; provider: string; log: string; url?: string; pullRequestUrl?: string } {
  const run = getAgentJob(repoRoot, runId);
  if (run.provider === 'github' && run.github) {
    const log = getGitHubAgentSessionLog(repoRoot, run.github.owner, run.github.repo, run.github.taskId, follow);
    return { runId, provider: 'github', log, url: run.github.url, pullRequestUrl: run.github.pullRequestUrl };
  }
  ...
  return { runId, provider: 'local', log: [readLog(run.stdoutPath), readLog(run.stderrPath)].filter(Boolean).join('\n') };
}

export function cancelAgentJob(repoRoot: string, runId: string): AgentJobMeta {
  const current = getAgentJob(repoRoot, runId);
  if (current.provider === 'github') throw new Error('GitHub cloud sessions must currently be cancelled from the GitHub Agents UI; the public agent-task API does not expose a stable cancellation contract here.');
  if (!['queued', 'running', 'unknown'].includes(current.status)) return current;
  const terminate = (pid: number | undefined): void => {
    ...
    try {
      ...
      try { process.kill(pid, 'SIGTERM'); } catch (_nested) { /* already exited */ }
    }
  };
  ...
  appendAgentJobEvent(repoRoot, runId, { type: 'run_cancelled', message: 'Local Run cancelled.' });
  try { updateTask(repoRoot, meta.issueId, meta.taskId, { status: 'cancelled', note: `${runId} cancelled.` }); } catch (_error) { /* ignore */ }
  return meta;
}
...
export function retryAgentJob(
  ...
  if (!['failed', 'cancelled', 'unknown', 'waiting_for_user'].includes(previous.status)) {
  ...
  const timeoutMs = normalizeAgentTimeoutMs(
    options.timeoutMs,
    { defaultMs: Math.max(previous.timeoutMs ?? 0, DEFAULT_AGENT_TIMEOUT_MS), maxMs: MAX_AGENT_TIMEOUT_MS },
  );
  return startTaskJob({
    repoRoot,
    ...
    githubRepo: previous.github ? `${previous.github.owner}/${previous.github.repo}` : undefined,
...
export function markAgentJobIntegrated(repoRoot: string, runId: string, sessionId: string): AgentJobMeta {
  const current = getAgentJob(repoRoot, runId);
  if (current.provider !== 'local') throw new Error('GitHub cloud session changes are reviewed and merged through their pull request, not local edit-session integration.');
  const { stdoutTail: _stdout, stderrTail: _stderr, ...meta } = current;
  meta.integratedSessionId = sessionId;
  meta.integratedAt = new Date().toISOString();
  writeJson(metaPath(repoRoot, runId), meta);
  appendAgentJobEvent(repoRoot, runId, { type: 'run_integrated', message: `Integrated through edit session ${sessionId}.` });
  return meta;
}
