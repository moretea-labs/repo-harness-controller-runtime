...
新用户从 [中文教程：连接 ChatGPT](docs/tutorials/02-connect-chatgpt.zh-CN.md) 开始。连接后先调用 `rh_status`，再用 `rh_context` 获取当前仓库上下文。默认 `advanced` 是固定、可修复的工具 schema。日常优先使用 `rh_status`、`rh_access`、`rh_context`、`rh_work` 和 `rh_inbox`；`rh_context` 会返回 repository-core、Git、Issue/Task、Campaign、Browser、iOS、Plugin、Evidence 和 Runtime Maintenance 等 capability 分组，帮助 ChatGPT 选择现有的强类型原子工具。当前 MCP transport 仍是静态 schema，不支持会话内按领域动态加载，因此 Direct Edit、命令、Git、Agent、Campaign、iOS、插件、artifact 和恢复工具继续保留。`core` 是兼容别名，`full` 是全量历史兼容工具面。仅当工具名称或 schema 真正变化时才需要重新扫描/连接；本次仅增加 capability 元数据，不改变 `tools/list`。
...
偶尔需要查看 CPU、打开应用或操作非仓库文件时，可使用 Controller 级 `local_system` 插件，不需要注册仓库。文件操作必须先授权一个有期限的目录目标，并拒绝路径逃逸、覆盖和删除。详见 [本机系统助手](docs/operations/local-system-assistant.zh-CN.md)。
